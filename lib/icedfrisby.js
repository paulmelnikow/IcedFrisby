...
Frisby.prototype._performInspections = function () {
  var i;
  for(i=0; i < this.current.inspections.length; i++) {
    var fn = this.current.inspections[i];
    fn.call(this);
  }
};
...
Frisby.prototype._makeRequest = function (mochaContext, done) {
  var self = this;
  var results = mochaContext.results_;
  ...
  var timeoutId = setTimeout(function maxWait(){
    ...
    if (mochaContext.tries < mochaContext.retries+1){
      ...
      results.totalCount = results.passedCount = results.failedCount = 0;
      results.skipped = false;
      results.items_ = [];

      process.stdout.write('R');
      self._makeRequest(mochaContext);
    } else {
...
// Invoke the expectations. Called from _makeRequest when request has finished
// successfully.
Frisby.prototype._invokeExpects = function (mochaContext, done) {
  ...
  for(var i=0; i < this.current.expects.length; i++) {
    if(false !== this._exceptionHandler) {
      try {
        this.current.expects[i].call(mochaContext);
      } catch(e) {
  ...
  this._wrapUp(null, done);
};

// Execute further expects for the current spec (called from assert())
Frisby.prototype._wrapUp = function(err, done) {
  if(this.current.after) {

    if (this.current.expectsFailed && this.current.outgoing.inspectOnFailure) {
      console.log(this.current.itInfo + ' has FAILED with the following response:');
      this.inspectStatus();
      this.inspectJSON();
    }
    ...
    for(i=0; i < this.current.after.length; i++) {
      var fn = this.current.after[i];
      if(false !== this._exceptionHandler) {
        try {
          fn.call(this);
        } catch(e) {
...
Frisby.prototype.toss = function(retry) {
  ...
  describe(self.current.describe, function() {
    it("\n\t[ " + self.current.itInfo + " ]", function(done) {
      ...
      mochaContext.retries = (self.current.outgoing.method.toUpperCase() == "POST") ? 0 : self.current.retry;
